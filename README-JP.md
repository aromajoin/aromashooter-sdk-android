...
`stopAllChambersWithIntensity()` は3.1.0で両方のコントローラーに追加されました（それ以前はSDKから正しい強度用の停止フレームを送れませんでした）。`stopAllChambers()` との違いは送信するプロトコルフレームの長さ（21バイト/15バイト）だけで、**どちらを呼んでもデバイスは停止します**。どのAPIで開始した噴射でも止められます。
